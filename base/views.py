...
def MBA_edit(request):
    return render(request,"edtior/MBA_edit.html",reguler_datas({'mentor':MBADB.objects.all()[::-1]}))
...
def Mentors_page_save(request): 
    mentor_name = request.POST.get("#Mentor_Name")
    mentor_special_at = request.POST.get("#Mentor_special_At")
    mentors_type = request.POST.get("#sub-heading3")
    mentor_image = request.FILES['#Mentor_image']
    obj = mentor_lists(menter_name=mentor_name, special_at=mentor_special_at, menter_img=mentor_image, TECK_OR_NONTECK=mentors_type)
    obj.save()
    return render(request, "pages/mentors_edit.html", reguler_datas())

def Mentors_page_edit(request):
    try:
        mentor_list = mentor_lists.objects.all()[::-1]
        return render(request,"pages/mentors_edit.html", reguler_datas({"mentor_lists":mentor_list}))
    except:
        print("maybe database are empty")
    return render(request, "pages/mentors_edit.html", reguler_datas())

def Mentor_page_delete(request):
    bl_id = request.POST.get("id")
...
def ourStartups_edit (request):
    try:
        ourStartups_logo = Ourstartup_images.objects.all()[::-1]
        return render(request,"pages/ourstartup_edit.html", reguler_datas({'startup':get_startup(), "ourStartups_logo":ourStartups_logo}))
    except:
        print("maybe database are empty")
...
def ourStartups_logo_save(request):
    Sub_heading = request.POST.get('#sub-heading1')
    image = request.FILES['#fileInput-single4']
    special_at = request.POST.get('#sub-heading2')
    pre_incbu_or_incbu = request.POST.get('#sub-heading3')
    obj = Ourstartup_images(Cmp_title=Sub_heading, image=image, special_at=special_at, pre_incbu_or_incbu=pre_incbu_or_incbu )
    obj.save()
    
    return render(request,"pages/ourstartup_edit.html", reguler_datas())

def delete_ourStartups_logo_save(request):
    bl_id = request.POST.get("id")
    logo = Ourstartup_images.objects.get(id=bl_id)
    logo.delete()
    return render(request,"pages/ourstartup_edit.html", reguler_datas())
